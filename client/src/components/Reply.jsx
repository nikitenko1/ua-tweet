...
const Reply = ({ handleClose, postId }) => {
  ...
  useEffect(() => {
    const getPost = async () => {
      ...
      data.retweetData
        ? setReplyToPost(data.retweetData)
        : setReplyToPost(data);
      setReplyToUser(data);
    };
    getPost();
    ...
    <TopModal onClose={handleClose}>
      ...
      <div className="flex-row ml-spacing mt-10 mr-10 mb-10" data-id={postId}>
        <div className="flex-col">
          <img
            src={replyToUser.postedBy?.profilePhoto.url}
            alt="userPic"
            width="40"
            height="40"
        ...
        <div className="flex-column flex-1 ml-spacing">
          <div className="flex-row">
            ...
            <span className="text-secondary ml-10">
              @{replyToUser.postedBy?.username}
            </span>
          </div>
          <div>
            {replyToPost.post?.text && (
              <p className="mt-10 mb-10 break-word">{replyToPost.post.text}</p>
            )}
