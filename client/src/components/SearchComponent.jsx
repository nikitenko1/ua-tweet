...
import { SearchIcon } from './forms/Icon';
let timer;

const SearchComponent = ({ handleFocus, handleSearchData }) => {
  const smScreen = useMediaQuery('(max-width:500px)');
    ...
    timer = setTimeout(async () => {
      if (search.trim() !== '') {
        ...
        const { data } = await axios.post(
          `/user/search?hashtag=true`,
          { search },
          config
        );
        setLoading(false);

        smScreen ? handleSearchData(data) : setSearchData(data);
      }
    }, 700);
  };
