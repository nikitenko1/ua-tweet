...
import { DELETE_POST } from '../redux/constants/postConstants';
import Popover from '@material-ui/core/Popover';
import { makeStyles } from '@material-ui/core/styles';
import { getPostTime, getTimeAndDate } from './utils/timeAndDate';
import { textUrl } from './utils/textToUrl';
...
const CreatePost = ({ post, history }) => {
  ...
  const handleLike = async (e) => {
    try {
      const { data } = await axios.put(`/api/post/${postId}/like`, {}, config);
      e.target.closest('.likeContainer').querySelector('span').innerText =
        data.likes.length || '';
  ...
  const handleRetweet = async (e) => {
    try {
      const { data } = await axios.post(
        `/api/post/${postId}/retweet`,
        {},
        config
      );

      e.target.closest('.retweetContainer').querySelector('span').innerText =
  ...
  const handleDelete = async (e) => {
    try {
      const { data } = await axios.delete(`/api/post/${postId}`, config);
  ...
  const handlePinAndUnpin = async (e) => {
    const pinned =
      e.target.closest('.pinned').getAttribute('data-pinned') === 'true'
        ? true
        : false;
    ...
    try {
      ...
      setAlertMessage(
        !pinned
          ? 'Tweet was pinned to your profile'
          : 'Tweet was unpinned from your profile'
      );
      setAlert(true);
    } catch (err) {
      if (err?.response.status === 401) {
  ...
  const handlePost = (e) => {
    if (!e.target.closest('.history_push_postId'))
  ...
  return (
    <>
      <div className="postHover pointer" onClick={handlePost}>
        {isRetweet &&
          (userRetweeted ? (
            <span className="flex items-center ml-35 pt-10">
              ...
              <span className="text-secondary ml-5 font-700 font-2sm ">
                You Retweeted
              </span>
            </span>
          ) : (
        ...
        {postData.pinned && (
          <div className="flex-row items-center ml-35 pt-10">
            <PinFillIcon className="fill-secondary svg-15" />{' '}
            <span className="text-secondary ml-5 font-700 font-2sm">
              Pinned Tweet
            </span>
          </div>
        ...
        <div
          className="flex-row ml-spacing pt-10 mr-10 mb-10 "
          data-id={postId}
        >
          <img
            src={postedUser.profilePhoto.url}
            alt="userPic"
            width="40"
            height="40"
            className="border-round history_push_postId"
          />

          <div className="flex-column flex-1 ml-spacing">
            <div className="flex-row justify-between items-center">
              <div className="history_push_postId">
                <Link to={`/profile/${postedUser.username}`}>
                  <span className=" flex-row flex-wrap ">
                    ...
                    <span style={{ margin: '4px 0 0 2px' }}>
                      {postedUser.role === 'admin' && (
                        <VipIcon className="svg-15" />
                      )}
                    </span>
                    ...
                    <ToolTip
                      title={getTimeAndDate(new Date(postData.createdAt))}
                    >
                      <span className="text-secondary">
                        {getPostTime(new Date(), new Date(postData.createdAt))}
                      </span>
                    </ToolTip>
                  </span>
                </Link>
              ...
              {postedUser._id === userInfo.id && (
                <div
                  className="pointer history_push_postId"
                  onClick={handleClick}
                >
            ...
            {isReply && (
              <span className="text-secondary font-2sm history_push_postId">
                Replying to{' '}
                <Link to={`/profile/${replyToUsername}`}>
                  <span className="text-primary">{`@${replyToUsername}`}</span>
                </Link>
              </span>
            )}
                ...
                if (s.type === 'text')
                  ...
                  return (
                    <Link to={`/hashtag/${s.text.slice(1)}`} key={i}>
                      <span className="text-primary history_push_postId">
                        {s.text}
                      </span>
                    </Link>
                  );
            ...
            <div className="flex-row justify-around">
              <div
                className="flex-row items-center history_push_postId"
                style={{ width: '70px' }}
              >
                <ToolTip title="Reply">
                  <div
                    className="inline-block pointer p-7 border-round reply"
              ...
              <div
                className={`flex-row items-center retweetContainer history_push_postId ${retweetActive}`}
                style={{ width: '70px' }}
              ...
              <div
                className={`flex-row items-center likeContainer history_push_postId ${likeActive}`}
                style={{ width: '70px' }}
              >
                <ToolTip title={likeTooltip}>
                  <div
                    className="inline-block pointer p-7 border-round like "
                    onClick={handleLike}
                  >
      ...
      {replyModal && (
        <Reply postId={postId} handleClose={() => setReplyModal(false)} />
      )}
      ...
      {pinModal && (
        <Modal onClose={() => setPinModal(false)}>
          <div
            className="ml-10 mr-10 mt-20 mb-20 text-center pinned"
            style={{ maxWidth: '280px' }}
            data-pinned={postData.pinned}
          >
      ...
      >
        <div className="bg-root text-secondary">
          <div
            className="flex-row items-center p-10 postHover pointer"
            onClick={() => {
              setDeleteModal(true);
              handleClose();
            }}
          >
            <div className="mt-5 mr-10">
              <DeleteIcon className="svg-20 fill-error" />
            </div>
          ...
          >
            <div className="mt-5 mr-10">
              <PinIcon className="fill-secondary svg-20" />
            </div>
            <span>
              {postData.pinned ? 'Unpin from Profile' : 'Pin to your Profile'}
            </span>
          </div>
          <div
            className="flex-row items-center p-10 postHover pointer"
          ...
          >
            <div className="mt-5 mr-10">
              <AddBookmarkIcon className="fill-secondary svg-20" />
            </div>
            <span>
              {bookmark
