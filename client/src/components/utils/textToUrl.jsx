...
export function textUrl(textToCheck) {
  ...
  while ((match = regex.exec(text)) != null) {
    ...
    splitText.push(
      ...checkHashTag(text.substr(startIndex, match.index - startIndex))
    );
  ...
  if (startIndex < text.length)
    splitText.push(...checkHashTag(text.substr(startIndex)));
...
function checkHashTag(input) {
  ...
  while ((match = re.exec(input)) != null) {
    result.push({
      text: input.substr(startIndex, match.index - startIndex),
      type: 'text',
    });
    result.push({
  ...
  if (startIndex < input.length)
    result.push({ text: input.substr(startIndex), type: 'text' });
  return result;
}
