...
import { CloseIcon, SearchIcon } from '../../components/forms/Icon';

let timer;

const NewMessage = ({ handleClose, history }) => {
  ...
  useEffect(() => {
    selectedUsers.length >= 1 ? setDisable(false) : setDisable(true);
  }, [selectedUsers]);
  ...
  const removeSelectedUser = (e) => {
    const removeUserId = e.target
      .closest('.selectedUser')
        ...
        <div className="modalCloseButton ">
          ...
          <div className="p-10 flex-row items-center border-bottom">
            ...
            <input
              ...
              className="flex-1 p-10 font-sm border-none text-root bg-root"
            />
          </div>
          <div className="p-10 flex-row flex-wrap">
            {selectedUsers?.map((user) => (
              <div
                className="selectedUser flex-row items-center fit-content border-secondary border-semi-round mr-10 mt-5 p-4 bgHover"
                key={user._id}
                data-id={user._id}
              >
                <img
          ...
          <div className="mt-10">
            {searchData.length === 0 ? (
              ...
              <div>
                {searchData.map((user) => (
                  <div
                    className="flex-row pointer pl-10 pr-20items-center user"
                    key={user._id}
                    data-id={user._id}
