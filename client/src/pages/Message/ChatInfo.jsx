...
import { CloseIcon } from '../../components/forms/Icon';
import TextInput from '../../components/forms/TextInput';

const ChatInfo = ({ match }) => {
    ...
    <>
      <div>
        ...
        <div className="flex-row items-center postHover p-20">
          <div className="flex-1 font-600 font-sm">{chatName}</div>
          <span
            className="text-primary mr-20 pointer"
            onClick={() => setChatNameModal(true)}
          >
            Edit
          </span>
        </div>
        <div style={{ borderTop: '10px solid var(--lightColor)' }}></div>
        <div className="font-800 font-md p-10 border-bottom mb-10">People</div>
        {chat.users?.map((user) => (
          <div key={user._id}>
      ...
      {chatNameModal && (
        <TopModal onClose={() => setChatNameModal(false)}>
          <>
            ...
            <div className="mt-20">
              <img
                src={chat.chatProfilePic}
                alt="pp"
                className="border-round block m-auto"
                width="80"
                height="80"
              />
            </div>
            ...
            <div className="mt-20 mb-20">
              <TextInput
                ...
                maxLength="50"
                handleInput={(value) => setChatName(value)}
                value={chatName}
              />
            </div>
          </>
