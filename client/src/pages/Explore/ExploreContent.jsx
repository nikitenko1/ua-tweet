    ...
    <a href={news.url} target="_blank" rel="noreferrer">
      <div className="flex-row items-center p-10 postHover pointer border-bottom">
        <div className="flex-1 mr-20">
          <span className=" block text-secondary font-sm">
            {news.source.name}
          </span>
...
const ExploreContent = () => {
  ...
  const getSportsNews = useCallback(async () => {
    if (sports.length === 0) {
      ...
      const { data } = await axios.get(`/api/explore/sports`, config);
      dispatch({ type: 'SPORTS', payload: data.articles });
      dispatch({ type: 'EXPLORE_LIST_LOADING', payload: false });
    }
  }, [config, dispatch, sports]);

  const getHealthNews = async () => {
    if (health.length === 0) {
      dispatch({ type: 'EXPLORE_LIST_LOADING', payload: true });
      const { data } = await axios.get(`/api/explore/health`, config);
  ...
  const getBusinessNews = async () => {
    if (business.length === 0) {
      dispatch({ type: 'EXPLORE_LIST_LOADING', payload: true });
      const { data } = await axios.get(`/api/explore/business`, config);
  ...
  const getScienceNews = async () => {
    if (science.length === 0) {
      dispatch({ type: 'EXPLORE_LIST_LOADING', payload: true });
      const { data } = await axios.get(`/api/explore/science`, config);
          ...
          {value === 0 && (
            <>
              {sports?.map(
                (news) =>
                  news.url &&
                  news.urlToImage && <Html news={news} key={news.title} />
              )}
            </>
          )}
