...
import { CameraIcon, CloseIcon } from '../../components/forms/Icon';
import TextInput from '../../components/forms/TextInput';
import CoverPhoto from './CoverPhoto';
import ProfilePhoto from './ProfilePhoto';
import BigModal from '../../components/modals/BigModal';
import { getDate } from '../../components/utils/timeAndDate';
...
const EditProfile = ({ handleClose }) => {
  ...
  const handleEdit = async () => {
    if (name.trim() === '') console.log('error');
    else {
      setLoading(true);
    }
    try {
      const { data } = await axios.put(
        `/api/user/${userInfo.username}/edit`,
        {
          name,
          bio,
          location,
    ...
    <>
      <BigModal onClose={handleClose}>
        ...
        <div className="modalCloseButton">
          <div className="flex-row items-center">
            ...
            <button className="fill-button text-white" onClick={handleEdit}>
              Save
            </button>
          </div>
        </div>
        <div className="coverPhoto">
          <span className="center bgHover border-round p-7">
            <label htmlFor="coverPhoto">
              <CameraIcon className="fill-white pointer" />
            </label>
            <input
              type="file"
              id="coverPhoto"
        ...
        <div className="mt-20 pt-10">
          <TextInput
            ...
            placeholder="Bio"
            maxLength="100"
            handleInput={(value) => setBio(value)}
            value={bio}
          />
        </div>
        <div className="mt-20 pt-10">
          <TextInput
            ...
            maxLength="30"
            handleInput={(value) => setLocation(value)}
            value={location}
          />
        </div>
        <div className="mt-20 pt-10 mb-20">
          <TextInput
            ...
            placeholder="Website"
            maxLength="100"
            handleInput={(value) => setWebsite(value)}
            value={website}
          />
        </div>
      ...
      {coverPhotoModal && (
        <CoverPhoto
          ...
          handleClose={() => setCoverPhotoModal(false)}
        />
      )}
      {profilePhotoModal && (
        <ProfilePhoto
          ...
          username={userInfo.username}
          handleClose={() => setProfilePhotoModal(false)}
        />
      )}
    </>
  );
