...
function WhoToFollow() {
  ...
  useEffect(() => {
    const getUsers = async () => {
      setLoading(true);
      const { data } = await axios.get(`/api/user/connect?limit=4`, config);
  ...
  return (
    <div className="whoToFollow">
      <span className="font-xl font-700 block text-center mb-20">
        Who To Follow
      </span>
      ...
      <Link to="/connect">
        <span className="text-primary ml-20 ">more</span>
      </Link>
    </div>
  );
}
